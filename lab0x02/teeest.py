...
while ip < len(filestring):
    ...
    if a == 'o':
        var1 = combine2_four_times_signed()
        ip = var1
        instructions[ipp] = f"INDEX MOVE TO {ip}"
    if a == 'p':
        var1 = combine2()
        ...
        if var2 == 0:
            ip = ip + var1()
            instructions[ipp] = f"IF POP() == 0 THEN JUMP TO {ip}"
    if a == 'q':
    ...
    if a == 't':
        sys.stdout.write(chr(stack1.pop()))
...
for i in range(len(filestring)):
    if filestring[i] == "":
        continue
    print(f"{i:4d}   {filestring[i]}    {instructions[i]:35};")
